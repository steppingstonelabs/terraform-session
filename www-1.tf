resource "digitalocean_droplet" "www-1" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "/tmp/install.sh -c=${var.client} -s=${var.secret} -p=${var.db_pwd} -d='${var.django_secret}' -h=\"127.0.0.1,localhost,${digitalocean_droplet.www-1.ipv4_address}\" -k=${var.slack_token} -u=$USER"
    ]
  }
}
